---
hide_table_of_contents: false
title: "Empeiria"
---
import { Icon } from "@iconify/react"; // Import the entire Iconify library.

<div className="archived-notice-banner">
  <span className="badge-archive">ARCHIVE</span>
  <span className="notice-text">Network no longer actively validated · Services docs only</span>
</div>

<div className="h1-with-icon icon-empeiria">
# Empeiria 
</div>
<!-- deskripsi -->
Empeiria is the first End-to-End Verifiable Data Infrastructure (EVDI). It enables seamless web3 adoption through one-click deployment, empowering organizations with the data of the future.

<!-- Sosmed links -->

<a href="https://empe.io"><Icon icon="tabler:world-www" width="35" height="35" /></a>
<a href="https://x.com/empe_io"><Icon icon="ri:twitter-x-fill" width="35" height="35" /></a>
<a href="https://t.me/web3Empeiria"><Icon icon="bi:telegram" width="35" height="35" /></a>
<a href="https://explorer.shazoes.xyz/empeiria-testnet"><Icon icon="meteor-icons:search" width="35" height="35" /></a>

<!-- Endpouints -->

### Public Endpoints

<div className="endpoints">API: https://empeiria-testnet-api.shazoes.xyz</div>
<div className="endpoints">RPC: https://empeiria-testnet-rpc.shazoes.xyz</div>
<div className="endpoints">gRPC: empeiria-testnet-grpc.shazoes.xyz:443</div>
